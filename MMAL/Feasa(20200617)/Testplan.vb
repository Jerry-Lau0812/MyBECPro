...
Module modTestplan
    ...
    Dim FeasaCollectList As New List(Of FeasaCollect)
    ...
    Public Function Testplan() As Integer
        ...
        PCBDatamatrix = "16820202103C20103636"

        '获取产品号
        Call CheckVarient(ProductNo)

        If Myconf.GENERAL.UseCompleteCode = "1" Then
            PCBDatamatrix = PCBDatamatrix.Substring(0, 12) & ProductNo
            MsgPrintLog("UUT:" + NumSite.ToString + " Change ReflowCode to BitronCode Success", 0)
        Else
        ...
        '校准Feasa
        Call CheckCalibrationFeasa(Numbay, NumSite, ProductNo, PCBDatamatrix)
...
#Region "Function Test"
        ...
        If FailFlag = PASS Then
            If Myconf.GENERAL.GetBinCodeFromFujiTrax = "1" Then
                ...
                If TraxResult.Count > 0 Then
                    FailFlag = PASS
                    MsgPrintLog("UUT:" + NumSite.ToString + " Check FujiTrax PASS", 0)
                Else
                    FailFlag = FAIL
                    MsgPrintLog("UUT:" + NumSite.ToString + " Check FujiTrax FAIL", 0)
                End If
            Else
                FailFlag = PASS
            End If
    ...
    ''' <param name="ProgNo"></param>
    ''' <remarks></remarks>
    Public Sub LightONLED(Optional ByVal TaskLabel As String = "POWERON")
        If FailFlag = PASS Then
            Try
                Dim Res As Boolean = False
                FailFlag = RunAnlTaskLabel("", TaskLabel, TaskLabel)
                If FailFlag = PASS Then
                    Threading.Thread.Sleep(200)
                    MsgPrintLog("Light ON " & TaskLabel & " Success:" & NumSite & " " & ProgNo, 0)
                    FailFlag = PASS
                Else
                    MsgPrintLog("Light ON " & TaskLabel & " Failed:" & NumSite & " " & ProgNo, 0)
                    FailFlag = FAIL
                End If
            Catch
                FailFlag = FAIL
    ...
    Public Sub LightOFFLED(Optional ByVal TaskLabel As String = "POWEROFF")
        If FailFlag = PASS Then
            Try
                Dim Res As Boolean = False
                FailFlag = RunAnlTaskLabel("", TaskLabel, TaskLabel)
                If FailFlag = PASS Then
                    Threading.Thread.Sleep(200)
                    MsgPrintLog("Light OFF " & TaskLabel & " Success:" & NumSite & " " & ProgNo, 0)
                    FailFlag = PASS
                Else
                    MsgPrintLog("Light OFF " & TaskLabel & " Failed:" & NumSite & " " & ProgNo, 0)
                    FailFlag = FAIL
                End If
            Catch
                FailFlag = FAIL
    ...
    Public Function StartCalibrationLEDs(ByVal SiteNo As Integer, ByVal Code_Bitron As String, ByVal sPath As String, ByVal Part As Integer) As Boolean
        Dim Res As Boolean = False
            ...
            If Ver.CodeBitron = Code_Bitron Then 'Select Product(Correspond code bitron)
                For Each stp In Ver.Sites        'Select Site Steps
                    If stp.SiteNo = SiteNo Then
                        For Each pt In stp.PART
                            If pt.Index = Part Then
                                For Each j In pt.SITE
                                    ...
                                    Res = FeasaCalibrationAbsIntensity(j.Name, j.CH, CalAbsInt, j.EnableAbsInt)
                                    Res = FeasaCalibrationWav(j.Name, j.CH, CalAbsWav, j.EnableWav)
                                    Res = FeasaCalibrationCxCy(j.Name, j.CH, CalCx, CalCy, j.EnableCxCy)
                                Next
                            End If
                        Next
                    End If
    ...
    Public Function FeasaCalibrationAbsIntensity(ByVal FeasaName As String, ByVal Ch As Integer, ByVal ValiAbsint As Double, Optional ByVal EnableOrNot As Boolean = True)
        If Not EnableOrNot Then
            Return True
        End If
    ...
    Public Function FeasaCalibrationWav(ByVal FeasaName As String, ByVal Ch As Integer, ByVal ValiWave As Double, Optional ByVal EnableOrNot As Boolean = False)
        If Not EnableOrNot Then
            Return True
        End If
        If FeasaUsing <> FeasaName Then '如果前后两通道不为同一个Feasa,则更改feasa
    ...
    Public Function FeasaCalibrationCxCy(ByVal FeasaName As String, ByVal Ch As Integer, ByVal ValiCx As Double, ByVal ValiCy As Double, Optional ByVal EnableOrNot As Boolean = True)
        If Not EnableOrNot Then
            Return True
        End If
        If FeasaUsing <> FeasaName Then '如果前后两通道不为同一个Feasa,则更改feasa
    ...
    Public Sub CheckFeasa(ByVal BayNo As Integer, ByVal SiteNo As Integer, ByVal ProgNo As String, ByVal Barcode As String)
        Dim PartUsed As List(Of String)
        Dim Res As Boolean = False
        If FailFlag = PASS Then
            PartUsed = GetPartList(SiteNo, ProgNo) 'Get all used parts
            If PartUsed.Count > 0 Then
                Res = True
            Else
                Res = False
            End If
            If Res Then
                For Each Part In PartUsed
                    CheckSubFeasa(BayNo, SiteNo, ProgNo, Barcode, Part)
                Next
            Else
                FailFlag = FAIL
            End If
    ...
    Public Function GetPartList(ByVal SiteNo As Integer, ByVal Code_Bitron As String) As List(Of String)
        Dim f As List(Of String)
        Try
            Dim XX = From C In Myconf.VERSIONIS Where C.CodeBitron = Code_Bitron Select C
            If XX.Count > 0 Then
                Dim DD = From C In XX(0).Sites Where C.SiteNo = SiteNo Select C
                If DD.Count > 0 Then
                    f = (From C In DD(0).PART Select C.Index Distinct).ToList
                End If
            End If
        Catch
            MsgBox("Please check xml configuration in funzconf.xml - SiteNo " & SiteNo & " PART")
        End Try
        Return f
    End Function

    Public Function GetTaskLabel(ByVal SiteNo As Integer, ByVal Code_Bitron As String, ByVal Part As String) As String
        Dim RunTaskLabel As String = ""
        Try
            Dim XX = From C In Myconf.VERSIONIS Where C.CodeBitron = Code_Bitron Select C
            ...
            If XX.Count > 0 Then
                Dim DD = From C In XX(0).Sites Where C.SiteNo = SiteNo Select C
                If DD.Count > 0 Then
                    Dim EE = From C In DD(0).PART Where C.Index = Part Select C

                    If EE.Count Then
                        RunTaskLabel = EE(0).TaskLabel
                    End If
                End If
            End If
    ...
    Public Sub CheckSubFeasa(ByVal BayNo As Integer, ByVal SiteNo As Integer, ByVal ProgNo As String, ByVal Barcode As String, ByVal Part As Integer)
        Dim FeasaUsed As List(Of String)
        Dim FeasaAddr As Integer
        'MsgPrintLog(ProgNo, 0)
        If FailFlag = PASS Then

            'Power ON LED by Part Task Label
            Dim PowerOnLabel As String = ""
            Dim PowerOFFLabel As String = ""
            Dim RuntaskLabel As String = GetTaskLabel(SiteNo, ProgNo, Part)

            PowerOnLabel = RuntaskLabel
            PowerOFFLabel = RuntaskLabel.Replace("ON", "OFF")

            LightONLED(PowerOnLabel)

            FeasaUsed = GetAllUsedFeasa(SiteNo, ProgNo, Part)
            ...
            'Once feasa collect finish, must power off all led immediately.
            LightOFFLED(PowerOFFLabel)
            ...
            If Res Then
                StartCheckLEDs(SiteNo, ProgNo, Barcode, Part)
            End If
    ...
    Public Sub CheckCalibrationFeasa(ByVal BayNo As Integer, ByVal SiteNo As Integer, ByVal ProgNo As String, ByVal PCBMatrix As String)
        If FailFlag = PASS Then
            Dim CaliMatrix As String = ""
            If My.Computer.FileSystem.FileExists(AppDomain.CurrentDomain.BaseDirectory + "\MMAL\MatrixUUT" + NumSite.ToString + ".txt") = True Then
                CaliMatrix = My.Computer.FileSystem.ReadAllText(AppDomain.CurrentDomain.BaseDirectory + "\MMAL\MatrixUUT" + NumSite.ToString + ".txt")
                CaliMatrix = CaliMatrix.Substring(0, 12) & ProgNo
            End If

            If CaliMatrix <> PCBMatrix Then
                Return
            End If
            ...
            Dim PartUsed As List(Of String)
            PartUsed = GetPartList(SiteNo, ProgNo)
            If PartUsed.Count > 0 Then
                FailFlag = PASS
            Else
                FailFlag = FAIL
                Return
            End If
            For Each pt In PartUsed
                Call StartSubCalibrationFeasa(BayNo, SiteNo, ProgNo, CaliFile, pt)
            Next
        Else
        End If
    ...
    Public Sub StartSubCalibrationFeasa(ByVal BayNo As String, ByVal SiteNo As Integer, ByVal ProgNo As String, ByVal CaliFile As String, ByVal Part As String)
        Dim FeasaUsed As List(Of String)
        Dim FeasaAddr As Integer
        If FailFlag = PASS Then
            FeasaUsed = GetAllUsedFeasa(SiteNo, ProgNo, Part)
            ...
            Call LightONLED(PowerOnLabel)


            Dim Res = SystemOperate.Device_FEASA.OpenCom(FeasaAddr)

            If Res Then
                MsgPrintLog(FeasaAddr & " Com Open Success!", 0)
            Else
                MsgPrintLog(FeasaAddr & " Com Open Failed!", 0)
            End If
            ...
            If Res Then
                StartCalibrationLEDs(SiteNo, ProgNo, CaliFile, Part)
            End If
            ...
            Call LightOFFLED(PowerOFFLabel)

            If Res Then
                FailFlag = PASS
                MsgPrintLog("Bay:" & BayNo & " NumSite:" & SiteNo & " FeasaComAddr:" & FeasaAddr & "Calibration Success", 0)
            Else
                FailFlag = FAIL
                MsgPrintLog("Bay:" & BayNo & " NumSite:" & SiteNo & " FeasaComAddr:" & FeasaAddr & "Calibration Failed", 0)
            End If
        End If
    End Sub
    ...
    Public Function StartCheckLEDs(ByVal SiteNo As Integer, ByVal Code_Bitron As String, ByVal PCBMatrix As String, ByVal Part As String) As Boolean
        For Each Ver In Myconf.VERSIONIS
            If Ver.CodeBitron = Code_Bitron Then 'Select Product(Correspond code bitron)
                For Each stp In Ver.Sites        'Select Site Steps
                    If stp.SiteNo = SiteNo Then
                        For Each pt In stp.PART
                            If pt.Index = Part Then
                                For Each j In pt.SITE
                                    For Each FeasaCollect In FeasaCollectList
                                        If FeasaCollect.FeasaName = j.Name Then
                                            ...
                                            If Myconf.GENERAL.GetBinCodeFromFujiTrax = "0" Then
                                                ValidateVF(j.LEDName, SiteNo, j.BinName, j.Code, TempRegisterVF, j.VFOffset, j.EnableVF)
                                                ValidateHSI(j.LEDName, SiteNo, j.BinName, j.Code, FeasaCollect.H(Ch), FeasaCollect.S(Ch), FeasaCollect.I(Ch), j.HueOffset, j.SatOffset, j.IntensityOffset, j.EnableHue)
                                                ValidateAbsIntensity(j.LEDName, SiteNo, j.BinName, j.Code, FeasaCollect.AbsI(Ch), j.IntensityOffset, j.EnableAbsInt)
                                                ValidateWavelength(j.LEDName, SiteNo, j.BinName, j.Code, FeasaCollect.Wave(Ch), j.DominantWavOffset, j.EnableWav)
                                                ValidateCXCY(j.LEDName, SiteNo, j.BinName, j.Code, FeasaCollect.Cx(Ch), FeasaCollect.Cy(Ch), j.CxOffset, j.CyOffset, j.EnableCxCy)
                                            Else
                                                Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Sub ValidateVF(ByVal LEDName As String, ByVal NumSite As Integer, ByVal BinName As String, ByVal Code As String, ByVal FeasaGetVF As Double, Optional ByVal Offset_Vf As Double = 0, Optional ByVal EnableOrNot As Boolean = True)
        If Not EnableOrNot Then
            Return
        End If
        Dim Res As Boolean = False
        If FailFlag = PASS Then
            ...
            If xx.Count > 0 Then
                ...
                If t(0).VF_SH > TerminalVF And t(0).VF_SL < TerminalVF Then
                    PrintResultMsg(PASS, NumSite, LEDName, BinName, TerminalVF, t(0).VF_SH.ToString, t(0).VF_SL.ToString, "_VF")
                    Res = True
                Else
                    PrintResultMsg(FAIL, NumSite, LEDName, BinName, TerminalVF, t(0).VF_SH.ToString, t(0).VF_SL.ToString, "_VF")
                    Res = False
                End If
            Else
                MsgPrintLog("UUT:" + NumSite.ToString & LEDName & " Not found " & Code & "in Node LED in your funzconf.xml", 0)
            End If
        Else
    ...
    ''' <remarks></remarks>
    Private Sub PrintResultMsg(ByVal TestFlag As Boolean, ByVal NumSite As Integer, ByVal LEDName As String, ByVal BinName As String, ByVal Value As Object, ByVal Hi_Limit As Object, ByVal Lo_Limit As Object, ByVal TestItem As String)
        If TestFlag = PASS Then
            MsgPrintLog("UUT:" & NumSite.ToString & " " & LEDName & " " & BinName & " " & TestItem & " PASS", 0)
            ObsDatalogTest(1, LEDName & " " & BinName, TestItem, PASS, Value.ToString, Lo_Limit.ToString, Hi_Limit.ToString, "0,0", NumSite)
        Else
            MsgPrintLog("UUT:" & NumSite.ToString & " " & LEDName & " " & BinName & " " & TestItem & " FAIL", 0)
            PrintRemarkFail(1, LEDName + TestItem, "0 0", Value.ToString, Hi_Limit.ToString, Lo_Limit.ToString, " NA", NumSite)
            ObsDatalogTest(1, LEDName & " " & BinName, TestItem, FAIL, Value.ToString, Lo_Limit.ToString, Hi_Limit.ToString, "0,0", NumSite)
            SiteResultWrite(NumSite, RESULT_FAIL) : FailFlag = RESULT_FAIL
        End If
    End Sub

    Private Sub PrintResultMsg(ByVal TestFlag As Boolean, ByVal NumSite As Integer, ByVal LEDName As String, ByVal BinName As String, ByVal Value_Cx As Object, ByVal Value_Cy As Object, ByVal Hi_Limit As Object, ByVal Lo_Limit As Object, ByVal TestItem As String)
        If TestFlag = PASS Then
            MsgPrintLog("UUT:" & NumSite.ToString & " " & LEDName & " " & BinName & " " & TestItem & " PASS", 0)
            ObsDatalogTest(1, LEDName & " " & BinName, TestItem, PASS, Value_Cx.ToString, Lo_Limit.ToString, Hi_Limit.ToString, "0,0", NumSite)
            ObsDatalogTest(1, LEDName & " " & BinName, TestItem, PASS, Value_Cy.ToString, Lo_Limit.ToString, Hi_Limit.ToString, "0,0", NumSite)
        Else
            MsgPrintLog("UUT:" & NumSite.ToString & " " & LEDName & " " & BinName & " " & TestItem & " FAIL", 0)
            ...
            ObsDatalogTest(1, LEDName & " " & BinName, "_Cx", FAIL, Value_Cx.ToString, Lo_Limit.ToString, Hi_Limit.ToString, "0,0", NumSite)
            ObsDatalogTest(1, LEDName & " " & BinName, "_Cy", FAIL, Value_Cy.ToString, Lo_Limit.ToString, Hi_Limit.ToString, "0,0", NumSite)
            SiteResultWrite(NumSite, RESULT_FAIL) : FailFlag = RESULT_FAIL
        End If
    End Sub
            ...
            If xx.Count > 0 Then
                ...
                If t.Count > 0 Then
                    If t(0).Hue_SH >= TeminalHSI_H And t(0).Hue_SL <= TeminalHSI_H Then
                        Res = True
                        Call PrintResultMsg(PASS, NumSite, LEDName, BinName, TeminalHSI_H, t(0).Hue_SH, t(0).Hue_SL, "_HUE")
                    Else
                        Res = False
                        Call PrintResultMsg(FAIL, NumSite, LEDName, BinName, TeminalHSI_H, t(0).Hue_SH, t(0).Hue_SL, "_HUE")
                    End If

                    If Res Then
                        If t(0).Sat_SH >= TeminalHSI_S And t(0).Sat_SL <= TeminalHSI_S Then
                            Res = True
                            Call PrintResultMsg(PASS, NumSite, LEDName, BinName, TeminalHSI_S, t(0).Sat_SH, t(0).Sat_SL, "_SAT")
                        Else
                            Res = False
                            Call PrintResultMsg(FAIL, NumSite, LEDName, BinName, TeminalHSI_S, t(0).Sat_SH, t(0).Sat_SL, "_SAT")
                        End If
                    End If
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Sub ValidateAbsIntensity(ByVal LEDName As String, ByVal NumSite As Integer, ByVal BinName As String, ByVal Code As String, ByVal FeasaGetAbsint As Double, Optional ByVal Offset_Abs As Double = 0, Optional ByVal EnableOrNot As Boolean = True)
        If Not EnableOrNot Then
            Return
        End If
        Dim Res As Boolean = False
        If FailFlag = PASS Then
            ...
            If xx.Count > 0 Then
                ...
                If t(0).Intensity_SH >= TerminalAbs And t(0).Intensity_SL <= TerminalAbs Then
                    Res = True
                    PrintResultMsg(PASS, NumSite, LEDName, BinName, TerminalAbs, t(0).Intensity_SH, t(0).Intensity_SL, "_AbsIntensity")
                Else
                    Res = False
                    'MsgBox(Numbay & " " & NumSite & " " & FeasaGetAbsint)
                    PrintResultMsg(FAIL, NumSite, LEDName, BinName, TerminalAbs, t(0).Intensity_SH, t(0).Intensity_SL, "_AbsIntensity")
                End If
            Else
                MsgBox("HSI Config Error!")
            End If
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Sub ValidateWavelength(ByVal LEDName As String, ByVal NumSite As Integer, ByVal BinName As String, ByVal Code As String, ByVal FeasaGetWav As Double, Optional ByVal Offset_Wav As Double = 0, Optional ByVal EnableOrNot As Boolean = True)
        If Not EnableOrNot Then
            Return
        End If
        Dim Res As Boolean = False
        If FailFlag = PASS Then
            ...
            If xx.Count > 0 Then
                ...
                If t(0).DominantWav_SH >= TerminalWav And t(0).DominantWav_SL <= TerminalWav Then
                    Res = True
                    PrintResultMsg(PASS, NumSite, LEDName, BinName, TerminalWav, t(0).DominantWav_SH, t(0).DominantWav_SL, "_DominantWave")
                Else
                    Res = False
                    PrintResultMsg(FAIL, NumSite, LEDName, BinName, TerminalWav, t(0).DominantWav_SH, t(0).DominantWav_SL, "_DominantWave")
                End If
            Else
                MsgBox("HSI Config Error!")
            End If
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Sub ValidateCXCY(ByVal LEDName As String, ByVal NumSite As Integer, ByVal BinName As String, ByVal Code As String, ByVal FeasaGetCx As Double, ByVal FeasaGetCy As Double, Optional ByVal Offset_Cx As Double = 0, Optional ByVal Offset_Cy As Double = 0, Optional ByVal EnableOrNot As Boolean = True)
        If Not EnableOrNot Then
            Return
        End If
        Dim Res As Boolean = False
        ...
        If FailFlag = PASS Then
            ...
            If xx.Count > 0 Then
                ...
                If t.Count > 0 Then
                    ...
                    If Res Then
                        PrintResultMsg(PASS, NumSite, LEDName, FormatFeasaCxCy(TeminalCx), FormatFeasaCxCy(TeminalCy), t(0).CXCYRange(0).CX & "," & t(0).CXCYRange(0).CY & t(0).CXCYRange(1).CX & "," & t(0).CXCYRange(1).CY & t(0).CXCYRange(2).CX & "," & t(0).CXCYRange(2).CY & t(0).CXCYRange(3).CX & "," & t(0).CXCYRange(3).CY,
                                                               "-", "_Cxcy")
                    Else
                        PrintResultMsg(FAIL, NumSite, LEDName, FormatFeasaCxCy(TeminalCx), FormatFeasaCxCy(TeminalCy), t(0).CXCYRange(0).CX & "," & t(0).CXCYRange(0).CY & t(0).CXCYRange(1).CX & "," & t(0).CXCYRange(1).CY & t(0).CXCYRange(2).CX & "," & t(0).CXCYRange(2).CY & t(0).CXCYRange(3).CX & "," & t(0).CXCYRange(3).CY,
                                                                "-", "_Cxcy")
                    End If
    ...
    Public Function CollectFeasaValue(ByVal FeasaUse As List(Of String)) As Boolean
        ...
        'Dim ColFEASA As Integer = FeasaUse.Count
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function GetAllUsedFeasa(ByVal SiteNo As Integer, ByVal Code_Biton As String, ByVal PartIndex As String) As List(Of String)
        Dim XX = From C In Myconf.VERSIONIS Where C.CodeBitron = Code_Biton Select C
        If XX.Count > 0 Then
            Dim DD = From C In XX(0).Sites Where C.SiteNo = SiteNo Select C
            If DD.Count > 0 Then
                Dim f As List(Of String) = (From C In DD(0).PART(PartIndex - 1).SITE Select C.Name Distinct).ToList
                Return f
            End If
        End If
    End Function
...
#Region "Check Resistor"
    ...
    Public Function StartCheckRess(ByVal SiteNo As Integer) As Boolean
        For Each stp In myPara.TestResStep        'Select Site Steps
            If FailFlag = PASS Then
                Dim TempV As Double
                Dim Rsum As Double 'Totol Resistor Value
                Dim A As Double = -1 'Modulus
                Dim AA() As String
                Dim X As Double

                RunAnlTaskLabel("", stp.RunLabel, stp.RunLabel)
                DvmRead(DVM1, TempV, MU_CURR)
                Rsum = TempV / stp.Modulux '总电阻

                If stp.Formula.Contains("*") And stp.Formula.Contains("/") And stp.Formula.Contains("+") Then '(21.5*X)/(21.5+X)
                    AA = stp.Formula.Split("/")(0).Replace("(", "").Replace(")", "").Split("*")
                    For Each i In AA
                        If IsNumeric(i) Then
                            A = i
                            Exit For
                ...
                ElseIf (Not stp.Formula.Contains("*")) And (Not stp.Formula.Contains("/")) And stp.Formula.Contains("+") Then '(X+21.5)
                    AA = stp.Formula.Split("+")
                    For Each i In AA
                        If IsNumeric(i) Then
                            A = i
                            Exit For
                ...
                ElseIf (Not stp.Formula.Contains("*")) And (stp.Formula.Contains("/")) And (Not stp.Formula.Contains("+")) Then 'X/3
                    AA = stp.Formula.Split("/")
                    For Each i In AA
                        If IsNumeric(i) Then
                            A = i
                ...
                ElseIf (Not stp.Formula.Contains("*")) And (Not stp.Formula.Contains("/")) And (Not stp.Formula.Contains("+")) Then 'X
                    X = Rsum
                End If
                ...
                If Myconf.GENERAL.GetBinCodeFromFujiTrax = "1" Then
                    BinName = GetBINAccordingLabel(stp.RunLabel.ToString.Replace("RES", "LED"))
                Else
                    BinName = stp.BinName
                End If

                ValidateRes(SiteNo, BinName, X, stp.RunLabel)
            Else
                FailFlag = FAIL
            End If
        Next
    ...
    Public Sub ValidateRes(ByVal NumSite As Integer, ByVal BinName As String, ByVal GetRes As Double, ByVal ResLabel As String)
        If FailFlag = PASS Then
            Try
                Dim TerminalRes As Double = GetRes
                Dim Res As Boolean = False

                Dim SRes_H As Double
                Dim SRes_L As Double

                Dim xx = From C In myPara.LEDRes Where C.BIN = BinName Select C
                If xx.Count > 0 Then

                    If xx(0).RESISTOR_H.ToString().Contains("%") Then
                        SRes_H = Val(xx(0).RESISTOR_S) * (1 + Val(xx(0).RESISTOR_H.ToString.Trim("%") / 100))
                    Else
                        SRes_H = Val(xx(0).RESISTOR_H)
                    End If
                    If xx(0).RESISTOR_L.ToString().Contains("%") Then
                        SRes_L = Val(xx(0).RESISTOR_S) * (1 - Val(xx(0).RESISTOR_L.ToString.Trim("%") / 100))
                    Else
                        SRes_L = Val(xx(0).RESISTOR_L)
                    End If

                    If SRes_H > TerminalRes And SRes_L < TerminalRes Then
                        PrintResultResMsg(PASS, NumSite, ResLabel & " " & BinName, TerminalRes, SRes_H, SRes_L, " ohm")
                        Res = True
                    Else
                        PrintResultResMsg(FAIL, NumSite, ResLabel & " " & BinName, TerminalRes, SRes_H, SRes_L, " ohm")
                        Res = False
                    End If
                End If
                If Res Then
...
#Region "Check Current"
    ...
    Public Function StartCheckCurrs(ByVal SiteNo As Integer) As Boolean
        For Each stp In myPara.TestCurStep        'Select Site Steps
            ...
            Dim BinName As String
            If Myconf.GENERAL.GetBinCodeFromFujiTrax = "1" Then
                BinName = GetBINAccordingLabelCur(stp.Combination, stp.RunLabel)
            Else
                BinName = stp.BinName
            End If
            Call ValidateCur(SiteNo, BinName, Cur)
        Next
    End Function

    Public Function GetBINAccordingLabelCur(ByVal StrCombination As String, ByVal StrRunLabel As String) As String
        Dim RetBin As String = ""
        ...
        For Each TC In TComBin
            ...
            Next
        Next
        Return RetBin.Trim("-") & "_" & StrRunLabel.Split("_")(1)
    End Function
    ...
    Public Sub ValidateCur(ByVal NumSite As Integer, ByVal BinName As String, ByVal GetRes As Double)
        If FailFlag = PASS Then
            Try
                Dim TerminalRes As Double = GetRes
                Dim Res As Boolean = False
                Dim SCur_H As Double
                Dim SCur_L As Double
                Dim xx = From C In myPara.CurLimit Where C.BIN = BinName Select C
                If xx.Count > 0 Then

                    If xx(0).T_H.ToString().Contains("%") Then
                        SCur_H = Val(xx(0).T_S) * (1 + Val(xx(0).T_H.ToString.Trim("%") / 100))
                    Else
                        SCur_H = Val(xx(0).T_H)
                    End If

                    If xx(0).T_L.ToString().Contains("%") Then
                        SCur_L = Val(xx(0).T_S) * (1 - Val(xx(0).T_L.ToString.Trim("%") / 100))
                    Else
                        SCur_L = Val(xx(0).T_L)
                    End If

                    If SCur_H > TerminalRes And SCur_L < TerminalRes Then
                        PrintResultCurMsg(PASS, NumSite, BinName, TerminalRes, SCur_H, SCur_L, " mA")
                        Res = True
                    Else
                        PrintResultCurMsg(FAIL, NumSite, BinName, TerminalRes, SCur_H, SCur_L, " mA")
                        Res = False
                    End If
                End If
                If Res Then
