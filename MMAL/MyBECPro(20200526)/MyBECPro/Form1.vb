...
Public Class Form1
                ...
                For Each stp In Ver.Sites        'Select Site Steps
                    If stp.SiteNo = SiteNo Then
                        For Each j In stp.SITE
                            ...
                            FeasaCalibrationAbsIntensity(j.Name, j.CH, CalAbsInt, j.EnableAbsInt)
                            FeasaCalibrationWav(j.Name, j.CH, CalAbsWav, j.EnableWav)
                            FeasaCalibrationCxCy(j.Name, j.CH, CalCx, CalCy, j.EnableCxCy)
                        Next
                    End If
                Next
            End If
    ...
    Public Function FeasaCalibrationWav(ByVal FeasaName As String, ByVal Ch As Integer, ByVal ValiWave As Double, Optional ByVal EnableOrNot As Boolean = True)
        If Not EnableOrNot Then
            Return True
        End If
        If FeasaUsing <> FeasaName Then '如果前后两通道不为同一个Feasa,则更改feasa
                        ...
                        For Each j In stp.SITE
                            ...
                            For Each FeasaCollect In FeasaCollectList
                                If FeasaCollect.FeasaName = j.Name Then
                                    ...
                                    Dim TempRegisterVF As Double = 0
                                    HoldRegisterRead(j.VFRegister, TempRegisterVF, "")
                                    ValidateVF(j.Name, SiteNo, j.BinName, j.Code, TempRegisterVF, j.VFOffset, j.EnableVF)
