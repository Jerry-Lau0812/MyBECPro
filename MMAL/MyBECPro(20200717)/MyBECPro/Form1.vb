...
Public Class Form1
    ...
    Private Sub InitialAllControls()
        Call InitialCaptureBrightLevel()
        Call InitialBoxFeasa()
        Call InitialConfigeration()
    End Sub

    Private Sub InitialComboPart(ByVal Code_Bitron As String, ByVal SiteNo As Integer)
        ...
        If XX.Count > 0 Then
            ...
            'Return f
            Dim P = From C In XX(0).Sites Where C.SiteNo = SiteNo Select C
            If P.Count > 0 Then
                For Each t In P(0).PART
                    ComboPart.Items.Add(t.Index)
                Next
            End If
        End If
    ...
    Private Sub InitialCaptureBrightLevel()
    ...
    Private Sub InitialBoxFeasa()
        ComboBoxFeasa.Items.Clear()
        Dim XX = MyConf.GENERAL.Name_Feasa

        For Each X In XX
            ComboBoxFeasa.Items.Add(X.Name)
        Next
        ComboBoxFeasa.SelectedIndex = 0
    End Sub

    Private Sub InitialConfigeration()
        ComboBitronCode.Items.Clear()
        Dim XX = MyConf.VERSIONIS

        For Each X In XX
            ComboBitronCode.Items.Add(X.CodeBitron)
        Next
        ComboBitronCode.SelectedIndex = 0
    End Sub
    ...
    Private Sub Btn_OpenCom_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim FeasaUsed As List(Of String) = GetAllUsedFeasa(SiteNo, CodeBitron, ComboPart.SelectedItem) 'Get All use feasa
    ...
    Private Sub Btn_Validate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_Validate.Click
        ...
        Dim PCBMatrix As String = TextBoxFuji.Text
        Dim PartIndex As String
        PartIndex = ComboPart.SelectedItem
        Dim CodeBitron = ComboBitronCode.SelectedItem 'Suport use CodeBitron 30100646
        Dim FeasaUsed As List(Of String) = GetAllUsedFeasa(Site, CodeBitron, PartIndex) 'Get All use feasa
        Dim FeasaAddr As Integer = GetUsedFeasaAddr(FeasaUsed) 'Get Addr
        ...
        PartUsed = GetPartList(SiteNo, CodeBitron)
        ...
        For Each t In PartUsed
            StartCheckLEDs(Site, CodeBitron, PCBMatrix, t)
        Next
    ...
    Private Sub Btn_StartCalibration_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_StartCalibration.Click
        Dim BayNo As String = 1
        Dim Site = SiteNo 'Support use Site No
        Dim CodeBitron = ComboBitronCode.Text
        Dim PartIndex As String
        PartIndex = ComboPart.SelectedItem
        Dim FeasaUsed As List(Of String) = GetAllUsedFeasa(Site, CodeBitron, PartIndex) 'Get All use feasa
        Dim FeasaAddr As Integer = GetUsedFeasaAddr(FeasaUsed) 'Get Addr
        ...
        Dim PartUsed As List(Of String)
        PartUsed = GetPartList(Site, CodeBitron)
        If PartUsed.Count > 0 Then
            Return
        Else
            MsgBox("Get Part Fail " & Site)
        End If

        For Each t In PartUsed
            StartSubCalibrationFeasa(BayNo, Site, CodeBitron, TextBoxFile.Text, t)
        Next
    ...
    Public Function StartCalibrationLEDs(ByVal SiteNo As Integer, ByVal Code_Bitron As String, ByVal sPath As String, ByVal Part As String) As Boolean
    ...
    Public Function FeasaCalibrationAbsIntensity(ByVal FeasaName As String, ByVal Ch As Integer, ByVal ValiAbsint As Double, Optional ByVal EnableOrNot As Boolean = True) As Boolean
    ...
    Public Function FeasaCalibrationWav(ByVal FeasaName As String, ByVal Ch As Integer, ByVal ValiWave As Double, Optional ByVal EnableOrNot As Boolean = True) As Boolean
        If Not EnableOrNot Then
            Return True
        End If
        If FeasaUsing <> FeasaName Then '如果前后两通道不为同一个Feasa,则更改feasa
    ...
    ''' <param name="Part">Part1 or Part2 or Part3...</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
                                        ...
                                        If FeasaCollect.FeasaName = j.Name Then
                                            ...
                                            Else
                                                ...
                                                Catch
                                                    ...
                                                    MsgPrintLog("Numsite" & SiteNo.ToString & " GetFujiTrx Failed! Please check FunzConfiguration or FujiTrax!", 0)
                                                End Try
                                                ValidateVF(j.LEDName, SiteNo, j.BinName, j.Code, TempRegisterVF, j.VFOffset, j.EnableVF)
                                                ValidateHSI(j.LEDName, SiteNo, j.BinName, j.Code, FeasaCollect.H(Ch), FeasaCollect.S(Ch), FeasaCollect.I(Ch), j.HueOffset, j.SatOffset, j.IntensityOffset, j.EnableHue)
                                                ValidateAbsIntensity(j.LEDName, SiteNo, j.BinName, j.Code, FeasaCollect.AbsI(Ch), j.IntensityOffset, j.EnableAbsInt)
    ...
    Public Function GetTraxResultAccording(ByVal SN As String, ByVal LEDName As String) As TraxMsg
        ...
        If LEDName = "" Then
            FailFlag = FAIL
            MsgPrintLog("Numsite" & SiteNo.ToString & " GetFujiTrx Failed!", 0)
            Return Trax
        End If
    ...
    Public Function ValidateAbsIntensity(ByVal LEDName As String, ByVal NumSite As Integer, ByVal BinName As String, ByVal Code As String, ByVal FeasaGetAbsint As Double, Optional ByVal Offset_Abs As Double = 0, Optional ByVal EnableOrNot As Boolean = True) As Boolean
        If Not EnableOrNot Then
            Return True
        End If
        If SiteResultRead(NumSite) = TEST_RESULT_PASS Then
    ...
    Public Function ValidateWavelength(ByVal LEDName As String, ByVal NumSite As Integer, ByVal BinName As String, ByVal Code As String, ByVal FeasaGetWav As Double, Optional ByVal Offset_Wav As Double = 0, Optional ByVal EnableOrNot As Boolean = True) As Boolean
        If Not EnableOrNot Then
            Return True
        End If
    ...
    Public Function ValidateCXCY(ByVal LEDName As String, ByVal NumSite As Integer, ByVal BinName As String, ByVal Code As String, ByVal FeasaGetCx As Double, ByVal FeasaGetCy As Double, Optional ByVal Offset_Cx As Double = 0, Optional ByVal Offset_Cy As Double = 0, Optional ByVal EnableOrNot As Boolean = True) As Boolean
        If Not EnableOrNot Then
            Return True
        End If
    ...
    Public Function ValidateVF(ByVal LEDName As String, ByVal NumSite As Integer, ByVal BinName As String, ByVal Code As String, ByVal FeasaGetVF As Double, Optional ByVal Offset_Vf As Double = 0, Optional ByVal EnableOrNot As Boolean = True) As Boolean
        If Not EnableOrNot Then
            Return True
        End If
...
#Region "Check Resistor"
    ...
    Public Function StartCheckRess(ByVal SiteNo As Integer) As Boolean
        For Each stp In myPara.TestResStep        'Select Site Steps
            ...
            If myPara.IsFujiTrax Then
                BinName = GetBINAccordingLabel(stp.RunLabel)
            Else
                BinName = stp.BinName
            End If
            ValidateRes(SiteNo, BinName, X)
        Next
...
#Region "Check Current"
    ...
    Public Function StartCheckCurrs(ByVal SiteNo As Integer) As Boolean
        For Each stp In myPara.TestCurStep        'Select Site Steps
            ...
            Dim BinName As String
            If myPara.IsFujiTrax Then
                BinName = GetBINAccordingLabelCur(stp.Combination)
            Else
                BinName = stp.BinName
            End If
...
#End Region
    ...
    Private Sub Btn_OpenCom_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_OpenCom.Click
        ...
        CodeBitron = ComboBitronCode.SelectedItem
        ...
        'Call InitialComboPart(ComboBitronCode.SelectedItem, SiteNo)
    End Sub
    ...
    Private Sub Btn_ChangeFeasa_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_ChangeFeasa.Click
        'Dim FeasaName As String = TextFeasaChange.Text
        Btn_ChangeFeasa.BackColor = Color.Yellow
        Application.DoEvents()
        Dim FeasaName As String = ComboBoxFeasa.SelectedItem
        Dim Res As Boolean = SystemOperate.Device_FEASA.Busfree
        Res = SystemOperate.Device_FEASA.BusgetFeasa(FeasaName)
        If Res Then
            Btn_ChangeFeasa.BackColor = Color.Green
